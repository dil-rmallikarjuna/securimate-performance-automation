...
function getToken() {
  ...
  if (response.status !== 200) {
    console.error(
      `Token fetch failed with status: ${response.status}, body: ${response.body}`
    );
...
export default function () {
  ...
  const requestPayload = {
    ...
    clientRequestId: `${
      selectedThirdParty.tpNum
    }-${new Date().valueOf()}-${Math.floor(Math.random() * 100 + 1)}`,
  ...
  ws.connect(wsUrl, {}, function (socket) {
    ...
    socket.on("open", function open() {
      ...
      startTime = new Date();
      socket.send(
        JSON.stringify({ action: "queryassist", data: requestPayload })
      );
    ...
    socket.on("message", function (message) {
      ...
      console.log(
        `Received message: ${message}, Response time: ${responseTime}ms`
      );
      const data = JSON.parse(message);
      check(data, {
        "response contains expected field": (obj) =>
          obj.body && obj.body.natural_language_response !== undefined,
      });
      socket.close();
    });
    ...
    socket.on("error", (e) =>
      console.error(
        `WebSocket error: ${e.error}, Details: ${JSON.stringify(e)}`
      )
    );
